...
class PemsBayDatasetLoader(object):
    ...
    def __init__(self, datadir=os.path.join(os.getcwd(), "data")):
        ...
        self.datadir = os.path.join(datadir, "pemsbay")
        self._load()
    ...
    def _load(self):
        ...
        if not os.path.isfile(
            os.path.join(self.datadir, "PEMS-BAY.zip")
        ):  # pragma: no cover
            if not os.path.exists(self.datadir):
                os.makedirs(self.datadir)
            self._download_url(url, os.path.join(self.datadir, "PEMS-BAY.zip"))

        if not os.path.isfile(
            os.path.join(self.datadir, "pems_adj_mat.npy")
        ) or not os.path.isfile(
            os.path.join(self.datadir, "pems_node_values.npy")
        ):  # pragma: no cover
            with zipfile.ZipFile(
                os.path.join(self.datadir, "PEMS-BAY.zip"), "r"
            ) as zip_fh:
                zip_fh.extractall(self.datadir)

        A = np.load(os.path.join(self.datadir, "pems_adj_mat.npy"))
        X = np.load(os.path.join(self.datadir, "pems_node_values.npy")).transpose(
            (1, 2, 0)
        )
