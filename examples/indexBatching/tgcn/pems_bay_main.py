...
def main():
    ...
    start = time.time()
    p1 = time.time() 
    indexLoader = PemsBayDatasetLoader(index=True)
    if allGPU:
    ...
    p2 = time.time() 
    t_mse, v_mse = train(train_dataloader, val_dataloader, mean[0], std[0], batch_size, epochs, edges, edge_weights, device, debug=debug)
    end = time.time()
